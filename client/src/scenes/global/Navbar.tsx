    ...
    <>
      <header className='container py-4 border-b border-gray-400 overflow-hidden'>
        ...
        >
          <div className='relative flex h-full flex-col  w-5/6 max-w-md bg-white  '>
            <div className='CROSS-ICON pt-3 pl-2 cursor-pointer'>
              <svg className='h-10 w-10 text-gray-600' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='1' strokeLinecap='round' strokeLinejoin='round'>
        ...
        <section className='MOBILE-MENU flex items-center justify-between md:hidden'>
          ...
          <div className='hover:opacity-50 cursor-pointer block' onClick={() => dispatch(setIsCartOpen())}>
            <Bag />
            <div className='absolute'>
              <p className='relative block left-5 bottom-10 bg-gray-100 w-5 h-5 text-center rounded-full'>{carts.length}</p>
            </div>
          </div>
        </section>
        <nav className=' '>
          <div className='DESKTOP-MENU hidden  md:flex justify-between items-center'>
            <div className='flex items-center'>
              ...
              <span className='text-xs ml-4'>by Natalie Kuklin</span>
              {/* <a href='/' className=''>
                <Logo />
              </a> */}
            </div>
            ...
            <h5 className='hover:text-gray-400 cursor-pointer' onClick={() => dispatch(setIsCartOpen())}>
              {`Cart ( ${carts.length} )`}
            </h5>
          </div>
        </nav>
