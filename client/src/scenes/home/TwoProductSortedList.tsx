...
export const TwoProductSortedList: React.FC<ITwoProductSortedListProps> = ({
  title,
  items,
  sort,
  firstItemId,
  ...
  const firstItem =
    currentItems.find(el => el.id === firstItemId) ||
    currentItems[currentItems.length - 1] ||
    null
  const secondItem =
    currentItems.find(el => el.id === secondItemId) ||
  ...
  return (
    <>
      ...
      <div className='grid grid-cols-2 gap-8 xl:gap-10 place-content-between'>
        {firstItem && (
          <Product item={firstItem} key={`${firstItem.id}`} isTrack={false} />
        )}
        {secondItem && (
          <Product item={secondItem} key={`${secondItem.id}`} isTrack={false} />
        )}
      </div>
      <button
